...
pub fn render_help_overlay(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let help_lines = vec![
        ...
        Line::from(vec![
            ...
            Span::raw(
                "  Launch an agent into the selected project (tmux split pane). N = wizard (prompt/resume)",
            ),
        ]),
        Line::from(vec![
        ...
        Line::from(vec![
            ...
            Span::raw("  git push / pull the selected project (fire-and-forget, non-blocking)"),
        ]),
        Line::from(vec![
